...
public abstract class RedisCache {
     ...
     * 重新加载策略
     * 
     * 取消兼容重新获取连接对象，如果需要多次操作redis数据库，请在方法的头部添加注解：RedisManager 
     * 或者 用如下代码包裹方法体： RedisCache.startDoIt() ... RedisCache.finishDoIt()  
     * 或者 使用doIt方法包裹原有方法体： RedisCache.doSomeThing(new Todo{实现doIt方法})
     * 
     * @param warnStr
     * @return
     * @see [类、类#方法、类#成员]
    ...
    private static int initStrategy(String warnStr) {
        if(RELOAD_STRATEGY || getIfDoSomeThing()){
            ...
            if(RELOAD_STRATEGY) logger.warn(warnStr);
            return dbIndex;
        }else{
            currentJedis.remove();
    ...
    public static void selectDB(int index){
        ...
        if(!"OK".equals(select)) setDBIndex(0);
    }
